...
export function _05_initAddModule() {
    ...
    toggleModule(false, _05_addCheckbox.closest('.rename-module'), module_elements);

    if (_05_addCheckbox) {
        _05_addCheckbox.addEventListener('change', function () {
            toggleModule(this.checked, _05_addCheckbox.closest('.rename-module'), module_elements);
            if (!this.checked) {
                ...
                applyAddOperation(); // Apply changes to all selected files when the module is enabled
            }
        });
    }
